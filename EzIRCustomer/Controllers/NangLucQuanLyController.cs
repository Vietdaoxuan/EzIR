...
{
    [RequiredLogin]
    public class NangLucQuanLyController : Controller
    {
        ...
        public NangLucQuanLyController(IConfiguration configuration, IAppLogger appLogger, IMemoryCache cache, IHostingEnvironment hostingEnvironment, ICommon common, IHttpService httpService, IHtmlLocalizer<SharedResource> localizer, IHttpContextAccessor httpContextAccessor, IErrorHandler errorHandler)
        {
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                ...
                if (!checklogin)
                {
                    _errorHandler.WriteStringToFile("CheckLoginSameTime:", "NangLucQuanLy");
        ...
        [HttpGet]
        [Route(LinkRoute.GET_NLQL)]
        public async Task<IActionResult> GetNlql(InfoSheetViewModel infoSheetViewModel)
        {
            try
            {
                ...
                responseObj = responseObj.Where(a => a.MenuID == ConstMenuID.NANG_LUC_QUAN_LY_QUAN_LY_CHAT_LUONG
                                                    || a.MenuID == ConstMenuID.NANG_LUC_QUAN_LY_QUAN_TRI_TAI_CHINH
                                                    || a.MenuID == ConstMenuID.NANG_LUC_QUAN_LY_QUAN_TRI_NHAN_SU
                                                    || a.MenuID == ConstMenuID.NANG_LUC_QUAN_LY_CAC_HE_THONG_QUAN_LY_KHAC
                                                    && a.Approve != 3).ToList();
        ...
        [HttpPost]
        [Route(LinkRoute.SAVE_NLQL)]
        public async Task<IActionResult> SaveNlql(List<InfoSheetViewModel> infoSheetViewModels)
        {
            try
            {
                // Kiểm tra xem có 2 user đăng nhập trên 2 thiết bị thì out user đầu tiên ra.
                ...
                if (!checklogin)
                {
                    _errorHandler.WriteStringToFile("CheckLoginSameTime:", "SaveNlql");
                    HttpContext.Session.Clear();
            ...
            string mess = "- Xem thông tin phê duyệt: Kinh doanh - Năng lực quản lý  ";
            ...
            foreach (var infoSheet in infoSheetViewModels)
            {
                ...
                changeInfos.Add(new ChangeInfo
                {
                    ...
                    KeyFunction = $"Năng lực quản lý - {title}",
                    Function = "Năng lực quản lý",
                    LevelID = "KT",
                    DetailLevelID = "NLQL"
                });
                ...
                infoSheet.RoleCode = RoleCode.NANG_LUC_QUAN_LY;
                infoSheet.Title = title;
                infoSheet.Language = _LANGUAGE;
            }
            _httpService.SetRoleControllerToHeader(RoleCode.NANG_LUC_QUAN_LY);
        ...
        [HttpPut]
        [Route(LinkRoute.EDIT_NLQL)]
        public async Task<IActionResult> EditNlql(List<InfoSheetViewModel> infoSheetViewModels)
        {
            try
            {
                // Kiểm tra xem có 2 user đăng nhập trên 2 thiết bị thì out user đầu tiên ra.
                ...
                if (!checklogin)
                {
                    _errorHandler.WriteStringToFile("CheckLoginSameTime:", "EditNlql");
                    HttpContext.Session.Clear();
        ...
        [HttpPost]
        [Route(LinkRoute.UPLOAD_IMAGE_NLQL)]
        public async Task<ActionResult> UploadImage(IFormFile upload)
        {
            try
            {
                ...
                if (!checklogin)
                {
                    _errorHandler.WriteStringToFile("CheckLoginSameTime:", "NangLucQuanLy_UploadImage");
                    HttpContext.Session.Clear();
        ...
        private string getTitleByMenuID(int? menuid)
        {
            switch (menuid)
            {
                case ConstMenuID.NANG_LUC_QUAN_LY_QUAN_LY_CHAT_LUONG:
                    {
                        return "Năng lực quản lý chất lượng";
                    }
                case ConstMenuID.NANG_LUC_QUAN_LY_QUAN_TRI_TAI_CHINH:
                    {
                        return "Năng lực quản lý tài chính";
                    }
                case ConstMenuID.NANG_LUC_QUAN_LY_QUAN_TRI_NHAN_SU:
                    {
                        return "Năng lực quản lý nhân sự";
                    }
                case ConstMenuID.NANG_LUC_QUAN_LY_CAC_HE_THONG_QUAN_LY_KHAC:
                    {
                        return "Năng lực quản lý các hệ thống quản lý khác";
                    }
