...
{
    [RequiredLogin]
    public class ThongTinDuAnDauTuController : Controller
    {
        ...
        public ThongTinDuAnDauTuController(IConfiguration configuration, IAppLogger appLogger, IMemoryCache cache, IHostingEnvironment hostingEnvironment, ICommon common, IHttpService httpService, IHtmlLocalizer<SharedResource> localizer, IHttpContextAccessor httpContextAccessor, IErrorHandler errorHandler)
        {
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                ...
                if (!checklogin)
                {
                    _errorHandler.WriteStringToFile("CheckLoginSameTime:", "ThongTinDuAnDauTu");
        ...
        [HttpGet]
        [Route(LinkRoute.GET_TTDADT)]
        public async Task<IActionResult> GetTtdadt(InfoSheetViewModel infoSheetViewModel)
        {
            try
            {
                ...
                responseObj = responseObj.Where(a => a.MenuID == ConstMenuID.THONG_TIN_DU_AN                                                    
                                                    && a.Approve != 3).ToList();
        ...
        [HttpPost]
        [Route(LinkRoute.SAVE_TTDADT)]
        public async Task<IActionResult> SaveTtdadt(List<InfoSheetViewModel> infoSheetViewModels)
        {
            try
            {
                // Kiểm tra xem có 2 user đăng nhập trên 2 thiết bị thì out user đầu tiên ra.
                ...
                if (!checklogin)
                {
                    _errorHandler.WriteStringToFile("CheckLoginSameTime:", "SaveTtdadt");
                    HttpContext.Session.Clear();
            ...
            string mess = "- Xem thông tin phê duyệt: Kinh doanh->Thông tin dự án đầu tư ";
            ...
            foreach (var infoSheet in infoSheetViewModels)
            {
                ...
                changeInfos.Add(new ChangeInfo
                {
                    ...
                    KeyFunction = $"Thông tin dự án đầu tư - {title}",
                    Function = "Thông tin dự án đầu tư",
                    LevelID = "KT",
                    DetailLevelID = "TTDADT"
                });

                infoSheet.EmailSettings = _emailSettings;
                infoSheet.ChangeInfos = changeInfos;
                infoSheet.Username = _USERNAME;
                infoSheet.RoleCode = RoleCode.THONG_TIN_DU_AN_DAU_TU;
                infoSheet.Title = title;
                infoSheet.Language = _LANGUAGE;

            }
            _httpService.SetRoleControllerToHeader(RoleCode.THONG_TIN_DU_AN_DAU_TU);
        ...
        [HttpPost]
        [Route(LinkRoute.UPLOAD_IMAGE_TTDADT)]
        public async Task<ActionResult> UploadImage(IFormFile upload)
        {
            try
            {
                ...
                if (!checklogin)
                {
                    _errorHandler.WriteStringToFile("CheckLoginSameTime:", "ThongTinDuAnDauTu_UploadImage");
                    HttpContext.Session.Clear();
        ...
        private string getTitleByMenuID(int? menuid)
        {
            switch (menuid)
            {
                case ConstMenuID.THONG_TIN_DU_AN:
                    {
                        return "Thông tin dự án đầu tư";
                    }
