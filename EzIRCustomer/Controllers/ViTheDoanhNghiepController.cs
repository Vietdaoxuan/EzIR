...
{
    [RequiredLogin]
    public class ViTheDoanhNghiepController : Controller
    {
        ...
        public ViTheDoanhNghiepController(IConfiguration configuration, IAppLogger appLogger, IMemoryCache cache, IHostingEnvironment hostingEnvironment, ICommon common, IHttpService httpService, IHtmlLocalizer<SharedResource> localizer, IHttpContextAccessor httpContextAccessor, IErrorHandler errorHandler)
        {
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                ...
                if (!checklogin)
                {
                    _errorHandler.WriteStringToFile("CheckLoginSameTime:", "ViTheDoanhNghiep");
        ...
        [HttpGet]
        [Route(LinkRoute.GET_VTDN)]
        public async Task<IActionResult> GetVtdn(InfoSheetViewModel infoSheetViewModel)
        {
            try
            {
                ...
                responseObj = responseObj.Where(a => a.MenuID == ConstMenuID.VI_THE_DOANH_NGHIEP_VI_THE_CHUNG
                                                    || a.MenuID == ConstMenuID.VI_THE_DOANH_NGHIEP_VI_THE_TRONG_TUNG_LINH_VUC_KINH_DOANH
        ...
        [HttpPost]
        [Route(LinkRoute.SAVE_VTDN)]
        public async Task<IActionResult> SaveVtdn(List<InfoSheetViewModel> infoSheetViewModels)
        {
            try
            {
                // Kiểm tra xem có 2 user đăng nhập trên 2 thiết bị thì out user đầu tiên ra.
                ...
                if (!checklogin)
                {
                    _errorHandler.WriteStringToFile("CheckLoginSameTime:", "SaveVtdn");
                    HttpContext.Session.Clear();
            ...
            string mess = "- Xem thông tin phê duyệt: Kinh doanh - Vị thế doanh nghiệp ";
            ...
            foreach (var infoSheet in infoSheetViewModels)
            {
                ...
                changeInfos.Add(new ChangeInfo
                {
                    ...
                    KeyFunction = $"Vị thế doanh nghiệp - {title}",
                    Function = "Vị thế doanh nghiệp",
                    LevelID = "KT",
                    DetailLevelID = "VTDN"
                });
                ...
                infoSheet.RoleCode = RoleCode.VI_THE_DOANH_NGHIEP;
                infoSheet.Title = title;
                infoSheet.Language = _LANGUAGE;
            }

            _httpService.SetRoleControllerToHeader(RoleCode.VI_THE_DOANH_NGHIEP);
        ...
        [HttpPut]
        [Route(LinkRoute.EDIT_VTDN)]
        public async Task<IActionResult> EditTtdadt(List<InfoSheetViewModel> infoSheetViewModels)
        {
            try
            {
                // Kiểm tra xem có 2 user đăng nhập trên 2 thiết bị thì out user đầu tiên ra.
                ...
                if (!checklogin)
                {
                    _errorHandler.WriteStringToFile("CheckLoginSameTime:", "EditTtdadt");
                    HttpContext.Session.Clear();
        ...
        [HttpPost]
        [Route(LinkRoute.UPLOAD_IMAGE_VTDN)]
        public async Task<ActionResult> UploadImage(IFormFile upload)
        {
            try
            {
                ...
                if (!checklogin)
                {
                    _errorHandler.WriteStringToFile("CheckLoginSameTime:", "ViTheDoanhNghiep_UploadImage");
                    HttpContext.Session.Clear();
        ...
        private string getTitleByMenuID(int? menuid)
        {
            switch (menuid)
            {
                case ConstMenuID.VI_THE_DOANH_NGHIEP_VI_THE_CHUNG:
                    {
                        return "Vị thế chung";
                    }
                case ConstMenuID.VI_THE_DOANH_NGHIEP_VI_THE_TRONG_TUNG_LINH_VUC_KINH_DOANH:
                    {
                        return "Vị thế trong từng lĩnh vực kinh doanh";
                    }
